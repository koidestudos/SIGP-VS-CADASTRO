  ...
  return `
    <div class="operational-dashboard">
      <div class="dash-header">
        ...
        ${hideNova ? '' : '<button class="btn btn-primary btn-lg" id="dash-nova">+ Nova Programação</button>'}
      </div>
      ...
      <div class="card mb-3">
        ...
        <div class="card-body table-compact">
          ${daSemana.length ? `<div class="table-wrapper"><table class="dash-prog-table">
            <thead><tr><th>Ação</th><th>Coordenação</th><th>Município</th><th>Gerência</th>${showAuthor ? '<th>Incluído por</th>' : ''}<th>Data inicial</th><th>Data final</th><th>Status</th><th></th></tr></thead>
            <tbody>${daSemana.map((p) => {
              ...
              <tr class="${getStatusRowClass(p.status)}">
                ...
                ${showAuthor ? `<td title="${String(p.criadoPorEmail || '').replace(/"/g, '&quot;')}">${autor.replace(/</g, '&lt;')}</td>` : ''}
      ...
      <div class="card mb-3">
        ...
        <div class="card-body table-compact">
          ${proximas.length ? `<div class="table-wrapper"><table class="dash-prog-table">
            <thead><tr><th>Ação</th><th>Coordenação</th><th>Município</th>${showAuthor ? '<th>Incluído por</th>' : ''}<th>Data inicial</th><th>Data final</th><th>Status</th><th></th></tr></thead>
        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="proximas-tabs">
              <div class="proximas-block"><h4>Hoje</h4>${renderAcaoList(hoje, 'Nada para hoje.', { showAuthor: showAuthor })}</div>
              <div class="proximas-block"><h4>Amanhã</h4>${renderAcaoList(amanha, 'Nada para amanhã.', { showAuthor: showAuthor })}</div>
              <div class="proximas-block"><h4>Próxima semana</h4>${renderAcaoList(semana, 'Nada na próxima semana.', { showAuthor: showAuthor })}</div>
            </div>
          </div>
        </div>
      ...
      <div class="card mb-3">
        ...
        <div class="card-body table-compact">
          ${ultimas.length ? `<div class="table-wrapper"><table class="dash-prog-table">
            <thead><tr><th>Ação</th><th>Coordenação</th>${showAuthor ? '<th>Incluído por</th>' : ''}<th>Status</th><th></th></tr></thead>
            <tbody>${ultimas.map((p) => {
              const coord = getCoordenacaoById(p.coordenacaoId);
              const autor = getIncluidoPorLabel(p) || '—';
...
export function bindDashboard(user) {
  ...
  document.querySelector('.operational-dashboard')?.addEventListener('click', (e) => {
    ...
    if (prog) showProgramacaoDetail(prog, { showAuthor: canSeeAuthor(user), showHistory: canSeeAuthor(user) });
  });
}
